...
function _profileBuildBlacklistPac(profile) {
    ...
    switch (profile.mode) {
        ...
        case 'socks4':
            ...
            break;
        case 'socks5':
            var sp = profile.socks_host + ':' + parseInt(profile.socks_port);
            proxy = 'SOCKS5 ' + sp + '; SOCKS ' + sp;
            break;
        default:
            proxy = 'DIRECT';
    }
...
function _profileBlacklistPacScript(proxy, patterns) {
    return [
        ...
        '// Hosts matching PATTERNS use PROXY; everything else goes DIRECT.',
        'var PROXY = ' + JSON.stringify(proxy + '; DIRECT') + ';',
